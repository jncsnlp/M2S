...
def eval_model(args,model_name,tokenizer, model, image_processor,context_len):
    ...
    conv.append_message(conv.roles[1], None)
    prompt = conv.get_prompt()
    #prompt = 'USER: <|image|>'+qs+'ASSISTANT: '
    ...
    #print(args.top_p)None
    #print(args.num_beams)1
    ...
    outputs = tokenizer.batch_decode(
        output_ids.sequences[:, input_token_len:], skip_special_tokens=True
    )[0]
    '''print(tokenizer.batch_decode(
        output_ids.sequences[:, input_token_len: input_token_len+1], skip_special_tokens=True
    ))'''
    '''print(outputs)'''

    '''probs = [torch.softmax(log.float(), dim=-1) for log in logits]
    #print(probs)
    ...
    #已生成的
    for i, token_id in enumerate(generated_ids[0][ input_ids.shape[1]:]):
    ...
    print(probs)'''
    #print(torch.softmax(output_ids,dim=-1))
    ...
    probs = torch.softmax(output_ids, dim=-1)
    outputs = tokenizer.batch_decode(
        output_ids[:, input_token_len:], skip_special_tokens=True
    )[0]'''
    '''print(tokenizer.batch_decode(
