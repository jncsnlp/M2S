...
def main(args):
    ...
    for j in range(map[dataset]): 
        ...
        for i in range(len(image_files)):
            ...
            output,logits=eval_model(args,model_name,tokenizer, model, image_processor,context_len)
            
            #POS Neg Ne
            ll=[logits[0][0,10321].float(),logits[0][0,12610].float(),logits[0][0,2448].float()]
